    ...
    */
    Quick.prototype.sort = function (arr, lo, hi) {
        ...
        */
        if (hi <= lo)
            return;
        var j = this.partition(arr, lo, arr.length - 1);
    ...
    Quick.prototype.less = function (A, B) { return A - B < 0; /* 这里可能需要做区分 */ };
    // 交换
    Quick.prototype.exch = function (arr, i, j) {
        var t = arr[i];
        arr[i] = arr[j];
        arr[j] = t;
    };
    Quick.prototype.show = function (arr) {
        for (var i = 0; i < arr.length; i++)
            console.log(arr[i] + '');
    };
    Quick.prototype.isSorted = function (arr) {
        for (var i = 0; i < arr.length; i++)
